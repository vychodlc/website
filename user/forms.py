...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(
        label='用户名或邮箱', 
        widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入你的用户名或邮箱地址',})
    )
...
class RegForm(forms.Form):
    username = forms.CharField(
        ...
        max_length=30,
        min_length=2,
        widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入你的用户名(3-30位)',}),
    )
    password = forms.CharField(
        label='密码',
        min_length=1,
        widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':'请输入你的密码(至少6位)',}),
    )
    password_again = forms.CharField(
        label='确认密码',
        min_length=1,
        widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':'请再次输入你的密码',}),
    )
    email = forms.EmailField(
        label='邮箱地址',
        max_length=30,
        min_length=3,
        widget=forms.EmailInput(attrs={'class':'form-control','placeholder':'请输入你的邮箱地址',}),
    )
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_password(self):
        ...
        if not self.user.check_password(old_password):
            raise forms.ValidationError('输入的旧的密码错误')
        return old_password
